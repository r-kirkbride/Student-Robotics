...
class robot:
    ...
    def getWallMarker(self, n, count = 0):
        ...
        closeToAdj = self.noMiddleMarkers[(self.cornerNum) % 4][3:]
        print(f"close to adj: {closeToAdj}")
        closeToHome = self.noMiddleMarkers[(self.cornerNum + 1) % 4][:3]
        ...
        while self.targetMarker == None:
                if n == 1:
                    ...
                    self.escape()
                    for i in range(len(fullMarkers)):
                        if fullMarkers[i].id in closeToAdj:
                            self.targetMarker = fullMarkers[i]
                    ...
                    fullMarkers = self.searchForWalls()
                    for i in range(len(fullMarkers)):
                        if fullMarkers[i].id in closeToHome:
                            self.targetMarker = fullMarkers[i]
                    
                    if self.targetMarker == None:
                        self.targetMarker = self.getWallMarker(2)
